    ...
    def update(self):
        ...
        if y_ray.distance <= 5:
            ...
            if self.driving or self.braking:
                ...
                if self.speed > 0:
                    volume1 = self.speed / 80 * self.volume
                    self.drive_sound.volume = clamp(volume1, 0, 1)
                elif self.speed < 0:
                    volume2 = -self.speed / 80 * self.volume
                    self.drive_sound.volume = clamp(volume2, 0, 1)

                if self.pivot_rotation_distance > 0:
                    volume3 = self.pivot_rotation_distance / 110 * self.volume
                    self.dirt_sound.volume = clamp(volume3, 0, 1)
                elif self.pivot_rotation_distance < 0:
                    volume4 = -self.pivot_rotation_distance / 110 * self.volume
                    self.dirt_sound.volume = clamp(volume4, 0, 1)
            else:
                self.drive_sound.volume = clamp(self.drive_sound.volume - 0.5 * time.dt, 0, 1)
                self.dirt_sound.volume = clamp(self.dirt_sound.volume - 0.5 * time.dt, 0, 1)
        ...
        if self.visible:
            if y_ray.distance <= self.scale_y * 1.7 + abs(movementY):
                ...
                if not self.hitting_wall:
                    self.rotation_parent.look_at(self.ground_normal, axis = Vec3.up)
